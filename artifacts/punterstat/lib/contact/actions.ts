...
<table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
  ...
  <tr><td style="padding:8px 0;color:#64748b;font-size:13px;">Email</td><td style="padding:8px 0;font-size:13px;"><a href="mailto:${safeEmail}" style="color:#0d9488;">${safeEmail}</a></td></tr>
...
<div style="background:#f8fafc;border-radius:10px;padding:16px;font-size:13px;color:#334155;line-height:1.7;white-space:pre-wrap;">${safeMessage}</div>
  ...
  // If the email service is misconfigured or down, surface it rather than
  // silently pretending the message was received.
  ...
  // Send user a confirmation — fire-and-forget; a failed confirmation
  // does not undo a successfully received inbox message.
  sendEmail({
    to: email,
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;border:1px solid #e2e8f0;overflow:hidden;">
        ...
        <tr><td style="padding:36px 40px;">
          ...
          <p style="margin:0;font-size:13px;color:#94a3b8;">
            In the meantime, our <a href="${appUrl}/faq" style="color:#0d9488;">FAQ</a> may have an immediate answer.
          </p>
        </td></tr>
