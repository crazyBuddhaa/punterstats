...
export async function getResolvedPredictions(userId: string): Promise<ResolvedPrediction[]> {
  const supabase = await createClient();
  const { data } = await supabase
...
export async function getPredictionRecords(userId: string): Promise<PredictionRecordRow[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("prediction_records")
    .select("*")
